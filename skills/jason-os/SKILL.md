...
- **Task lanes = now · next · later · someday** (time horizon). `now` = today, capped at ~5 **undated** tasks. A task with a `due_date` is moved into `now` automatically by code on its due date and is **exempt from the cap** — don't lane tasks by their due date yourself. Be conservative reordering; don't churn priorities; respect what Jason set by hand. `task_reorder` sets lane + position.
...
- **Tags = the project/goal axis.** Tasks carry `tags` (higher-level project/goal, e.g. 'PayPal POC', 'fundraising') — independent of lanes (time horizon). When you create/curate tasks, tag them by project/goal, and **reuse the existing vocabulary** (`task_tags`) instead of coining near-duplicate variants; `task_update` `tags` replaces the whole set. Keep tags few and meaningful. (The groomer auto-tags + normalizes nightly.)
- **Notes** are standalone markdown notes organized by the SAME project/goal tags as tasks (`note_create`/`notes_search`/`note_get`/`note_update`). A note tagged 'PayPal POC' belongs to that project alongside its tasks — that shared tag is how notes connect to tasks/CRM. Reuse the existing tag vocabulary.
- **Snooze ≠ due date — they're different clocks.** `due_date` = the deadline + calendar reminder. **Snooze** (`task_snooze`) = "hide this now/next task in `later` until I want to see it again," a board-triage tool that does NOT touch the calendar. A snoozed task with a due_date still shows its reminder. A snooze can't outlast the deadline (the wake is clamped to the due date; `task_unsnooze` wakes it now). Use snooze to defer something off the active board; use a due_date to set a real deadline.
